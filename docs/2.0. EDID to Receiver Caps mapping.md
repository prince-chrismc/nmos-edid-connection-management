...
Each video timing MUST be present in Receiver Capabilities as a Constraint Set with non-empty `urn:x-nmos:cap:format:frame_width`, `urn:x-nmos:cap:format:frame_height` and `urn:x-nmos:cap:format:grain_rate` fields. The timing descriptors MAY include `urn:x-nmos:cap:format:interlace_mode` when applicable.
...
Defined in [E-EDID][E-EDID] section 3.10, there a 4 possible descriptors which can be provided. The first, _Prefered Timing Mode_ is a requirement and MUST have a `urn:x-nmos:cap:meta:preference` value off `100`.
...
* `urn:x-nmos:cap:format:grain_rate` SHALL be accurately represented by
  ...
  * `denominator` MUST be calculated with _Horizontal Addressable Video in pixels_, HB - _Horizontal Blanking in pixels_, VAV - _Vertical Addressable Video in lines_ and VB - _Vertical Blanking in lines_
