...
Each video timing SHOULD be present in Receiver Capabilities as a Constraint Set with non-empty `urn:x-nmos:cap:format:frame_width`, `urn:x-nmos:cap:format:frame_height` and `urn:x-nmos:cap:format:grain_rate` fields. There are multiple E-EDID blocks which keep information about timings each with their own mapping requirements.
